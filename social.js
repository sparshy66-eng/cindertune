...
(function(){
  ...
  function clientUid(){
    // Prefer the real signed-in account's UID so friends/requests/presence follow
    // *you*, not just the one browser you happened to be using. Only falls back to
    // a random per-browser ID when Firebase isn't configured (offline demo mode).
    if(fbReady() && firebase.auth().currentUser){
  ...
  function clearPresence(){
    const uid = clientUid();
    if(fbReady()){
      firebase.database().ref('wf_presence/' + uid).remove();
    } else {
      const presence = LS.get(K_PRESENCE, {});
      delete presence[uid];
      LS.set(K_PRESENCE, presence);
  ...
  window.WallflowSocial = {
    ...
    setPresence, clearPresence, listenPresence, logActivity, listenActivity,
